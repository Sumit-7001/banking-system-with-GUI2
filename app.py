...
# ---------------- USERS DB ----------------
USERS_DB = {
    "Sumit": {"password": "sumit123", "full_name": "Sumit Sahoo", "image": "s1.jpg"},
    "Moumi": {"password": "moumi123", "full_name": "Moumi", "image": "moumi.jpeg"},
    "Rudra": {"password": "rudra123", "full_name": "Rudra", "image": "jeet.jpg"},
    "user1": {"password": "pass1", "full_name": "User One", "image": None},
    "user2": {"password": "pass2", "full_name": "User Two", "image": None}
}
...
# ---------------- CREATE ACCOUNT ----------------
import re  # Make sure this is at the top of your app.py
...
def create_account():
    if request.method == 'POST':
        try:
            import re  # make sure re is imported for regex

            # --- Get all form inputs safely ---
            name = request.form['name'].strip()
            acc_no = request.form['acc_no'].strip()
            balance = float(request.form['balance'])
            aadhar_no = request.form['aadhar_no'].strip()
            mail_id = request.form['mail_id'].strip()
            ...
            # ✅ EMAIL validation
            if not re.fullmatch(email_pattern, mail_id):
                flash("❌ Invalid Email Format! Example: user@example.com", 'danger')
                return redirect(url_for('create_account'))

            # ✅ ACCOUNT NUMBER validation (only digits, any length)
            if not acc_no.isdigit():
                flash("❌ Account Number must contain digits only (0–9)!", 'danger')
                return redirect(url_for('create_account'))
            ...
            created = excel_Day2.create_new_account(acc_no, name, balance, aadhar_no, mail_id, dob, address)
            if created:
            ...
            flash("❌ Please enter a valid numeric value for balance!", 'danger')
    ...
    if request.method == 'POST':
        try:
            ...
            if not os.path.exists(file_path):
                ...
                balance = excel_Day2.get_balance(file_path)
                flash(f"Current balance for {acc_no}: ₹{balance}", 'success')
        except Exception as e:
            flash(f"Error: {e}", 'danger')
